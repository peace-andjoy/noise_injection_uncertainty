...
def main():
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        alphas.append([param.item() for name, param in net.named_parameters() if 'alpha_' in name]) # 记录权重
    ...
    # alpha_names = [name for name, param in net.named_parameters() if 'alpha_' in name]

    # print(alphas)
    # plt.figure(figsize=(8, 3))
    # for i in range(len(alpha_names)):
    #     plt.plot([alpha[i] for alpha in alphas], label=alpha_names[i])
    # plt.legend()
    # plt.grid(True)
    # plt.savefig(os.path.join(args.save_path, "main_learned_alpha.png"))
    # plt.show()
