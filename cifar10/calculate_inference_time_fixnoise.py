...
num_filters = 4
args.ngpu = 1
args.folder = "cifar10_fixnoise_resnet8_4_160_SGD_notadv_0.02"
...
test_times_array = [1]
cost_time = []
for test_times in test_times_array:
    for package in os.listdir(args.resume_path):
        ...
        for input, target in test_loader:
            cal_val_time(input, target, net, test_times = test_times)
        end_time = time.time()
        print(args.folder, "total time cost:", end_time-start_time)
        break
    cost_time.append(end_time-start_time)

cost_time = np.array(cost_time)
np.savetxt("./save_inference_time/{}_cost_time_{}.txt".format(args.folder, datetime.now().strftime("%Y-%m-%d %H:%M:%S")), cost_time)
