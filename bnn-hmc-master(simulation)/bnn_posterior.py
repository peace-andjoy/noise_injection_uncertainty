...
num_sample = 20
np.random.seed(123)
x_train = np.random.uniform(-4, 4, num_sample)
...
if __name__ == '__main__':
    ...
    for chain in range(chains):
        ...
        post, es, info = run_HMC_sampler(n_hmc_iters=chain_length,
                                    init_bnn_params=nn_params, eps=0.005, n_leapfrog_steps=500)
        t2 = time.time()
        print('time:', t2-t1)
        print(info)
        energies.append(es)
    ...
    plt.plot(x_train, y_train, 'rx')
    plt.plot(x_grid, x_grid**3, linestyle='--')
    plt.plot(x_grid, mean_sample, 'k-')
    plt.gca().fill_between(x_grid.flat, mean_sample-3*std, mean_sample+3*std,
                           color="#dddddd")
    plt.savefig('posterior_samples.png', bbox_inches='tight')
    plt.show()
    print(np.sqrt(np.mean((x_grid**3-mean_sample)**2)))
    print(mean_standardized_log_loss(x_grid**3, mean_sample, std, sample_weight=None, multioutput="uniform_average", squared=True))