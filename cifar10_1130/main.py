...
parser.add_argument('--data_path', default='/home/xueqiong/CVPR_2019_PNI-master/data',
...
# fix technique
parser.add_argument('--fix_noise_level', type=float, default=0.1, help='effective only if arch is fixnoise_resnet')
parser.add_argument('--num_filters', type=int, default=16, help='number of filters')
parser.add_argument('--weight_decay_alpha', type=float, default=0, help='effective only if arch is noise_resnet, should be negative')
...
rdm = RandomState(args.manualSeed)
...
def main():
    # Init logger
    args.save_path = args.save_path+'/{}'.format(args.manualSeed)
    ...
    print([net.state_dict()[name] for name in net.state_dict().keys() if 'fix' in name])

    if args.optimizer == "SGD":
        print("using SGD as optimizer")
        optimizer = torch.optim.SGD([
                                    {'params': normal_param},
                                    {'params': alpha_param, 'weight_decay': args.weight_decay_alpha}
                                    ],
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        # save addition loss log for plotting
        loss_logger(base_dir=args.save_path,
                        epoch=epoch,
...
def train(train_loader, model, criterion, optimizer, epoch, log, attacker=None, adv_train=False):
    ...
    for i, (input, target) in enumerate(train_loader):
        ...
        if adv_train and (attacker is not None):
            ...
            loss = 0.5 * loss + 0.5 * loss_adv     # 损失函数
...
def loss_logger(base_dir, epoch, train_loss, test_loss):
    file_name = 'loss.txt'
    file_path = "%s/%s" % (base_dir, file_name)
    # create and format the log file if it does not exists
    if not os.path.exists(file_path):
    ...
    recorder = {}
    recorder['epoch'] = epoch
    recorder['train'] = train_loss
    recorder['test'] = test_loss
    # append the epoch index, train loss and test loss:
    with open(file_path, 'a') as loss_log:
        loss_log.write(
            '{epoch}       {train}    {test}\n'.format(**recorder))
